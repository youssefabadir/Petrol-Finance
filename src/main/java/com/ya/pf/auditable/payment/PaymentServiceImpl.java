...
public class PaymentServiceImpl implements PaymentService {
    ...
    public PaymentEntity validatePayment(PaymentEntity payment) throws MissingRequestValueException {
        ...
        boolean exists = paymentRepository.existsByNumberAndPaymentMethodIdAndPaymentTypeEquals(paymentNumber,
                                                                                                paymentMethodId,
                                                                                                paymentType);
        if (exists) {
        ...
        PaymentEntity paymentEntity = paymentRepository.findFirstByPaymentMethodIdAndDateLessThanEqualOrderByDateDescIdDesc(
            paymentMethodId,
            date);
        float treasuryBalance =
            paymentEntity == null ? config.getTreasuryBalance() : paymentEntity.getTreasury_balance();
        PaymentMethodEntity paymentMethod = paymentMethodService.getPaymentMethodById(paymentMethodId);
        float paymentMethodBalance =
            paymentEntity == null ? paymentMethod.getStartBalance() : paymentEntity.getPaymentMethodBalance();
    ...
    @Override
    public Page<PaymentEntity> getPayments(long paymentMethodId, int pageNo, int pageSize, String sortBy, String order,
        ...
        if (start == null || end == null) {
            ...
            if (paymentMethodId == -1) {
                return paymentRepository.findAllByDateBetween(Date.valueOf(start),
                                                              Date.valueOf(end.plusDays(1)),
                                                              pageable);
            } else {
                return paymentRepository.findByPaymentMethodIdAndDateBetween(paymentMethodId,
                                                                             Date.valueOf(start),
                                                                             Date.valueOf(end.plusDays(1)),
                                                                             pageable);
            }
    ...
    public void deletePaymentById(long id) {
        ...
        if (paymentRepository.existsById(id)) {
            ...
            if (isTransferred) {
                PaymentEntity transferredPayment = paymentRepository.findByNumberEqualsAndIdNot(paymentNumber,
                                                                                                paymentId);
                if (paymentType.equals("CUSTOMER_PAYMENT")) {
                    ...
                    customerTransactionService.deleteCustomerTransactionByPaymentId(customerPayment.getCustomer()
                                                                                                   .getId(),
                                                                                    paymentId,
                                                                                    payment.getAmount(),
                                                                                    payment.getDate());
                    ownerTransactionService.deleteOwnerTransactionByPaymentId(ownerPayment.getSupplier().getId(),
                                                                              transferredPayment.getId(),
                                                                              payment.getAmount(),
                                                                              payment.getDate());
                } else {
                    CustomerPaymentEntity customerPayment = customerPaymentRepository.getReferenceById(
                        transferredPayment.getId());
                ...
                paymentRepository.deleteByNumberAndPaymentMethodIdAndTransferredIsTrue(paymentNumber,
                ...
                if (paymentType.equals("CUSTOMER_PAYMENT")) {
                    ...
                    paymentMethodService.updatePaymentMethodBalance(payment.getPaymentMethodId(),
                                                                    ...
                                                                    payment.getAmount());
                    paymentRepository.updatePaymentMethodBalanceById(payment.getPaymentMethodId(),
                                                                     id,
                                                                     payment.getAmount() * -1,
                                                                     payment.getDate());
                } else {
